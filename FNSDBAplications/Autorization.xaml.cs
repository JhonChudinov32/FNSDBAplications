...
using SaveOptions = FNSDBAplications.connection.SaveOptions;
    ...
    public partial class Autorization : Window
    {
        ...
        public Autorization()
        {
            if (saveOptions == null)
            {
                ...
                if (File.Exists("SaveOptions.xml"))
                {
                    ...
                    {
                        var serializer = new XmlSerializer(typeof(connection.SaveOptions));
        ...
        private void Entered_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT COUNT(middlename)as qw FROM [dbo].[User] WHERE [login] ='" + Login.Text + "' and [password] ='" + Password.Password + "' and [isenabled] = 'true' ", Connect.cnn);
